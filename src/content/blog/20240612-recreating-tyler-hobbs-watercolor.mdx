...
Here we are using a value of $\frac{length}{4}$. This means that long edges have a greater chance of distorting _further_, while shorter edges will result in more subtle distortions. These are just a *starting point* for the types of functions you could use to derive these parameters, the possiblities are endless and can have a major impacts on the output of the algorithm.
