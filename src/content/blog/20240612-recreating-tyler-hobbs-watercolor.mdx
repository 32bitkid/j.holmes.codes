...
So far, this doesn't look very good. Using the same values for *every* edge doesn't make it look *distorted*; instead, we've what one might call a **star generator**, which is&hellip; not exactly what we were going for. However, what if instead of using static values for each new point we generate, what if we use some amount of randomization. We could add a little bit of randomness to each time we generate our three variables: **position**, **angle** and **magnitude**.
