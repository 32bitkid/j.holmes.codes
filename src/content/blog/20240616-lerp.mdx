...
Ouchies! Because the delta between $a$ and $b$ is greater that what a 64-bit floating point number can represent, it gets rounded *out* when doing $(b-a)$. As such, we end up getting a value *outside* the range of $[a,b]$. We get a result of $0$, which is *beyond* our expected limit of $10^{-8}$. As such, it's sometimes recommended to use this implementation instead:
